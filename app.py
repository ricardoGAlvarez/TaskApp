...
@app.before_request
def bloquear_endpoints():
    ruta_actual = request.endpoint
    if  ruta_actual != 'inicio' and not request.path.startswith('/static'):
...
def login():
    ...
    if _usuario != usuario or _password != password:
        return render_template('pages/index.html', error=True)
    else:
        ...
        conexion.commit()
        return render_template('pages/pendientes.html', reclamo=reclamo, usuario=_usuario)
...
def guardar():
    ...
    if result:
       ...
       return redirect('/nuevo')
        ...
        sql= "INSERT INTO   `reclamos` ( `id`, `nombre`, `barrio`,  `calle`,  `problema`,`estado`,`fecha`) VALUE(NULL,%s,%s,%s,%s,%s,%s);"
        datos=(_nombre,_barrio,_calle,_problema,_estado,_fecha )
        conexion= mysql.connect()
        cursor=conexion.cursor()
        ...
        datos_historial=(_problema, _fecha, reclamo_id)
...
@app.route('/addhistorial/<int:id>', methods=['GET'])
def verhistorial(id):
    conexion = mysql.connect()
    cursor = conexion.cursor()
    ...
    return render_template('pages/addhistorial.html', id=id, historial=historial)


@app.route('/addhistorial/guardar/<int:id>', methods=['POST'])
def addhistorial(id):
    fecha_actual = date.today()
    _estado=1
    _fecha=fecha_actual
    _reclamo_id=id
    _historial=request.form['txtHistorial'] + " (Realizado)"
...
def editar(id):
    ...
    return render_template('pages/edit.html', id=id, nombre=nombre, calle=calle, barrio=barrio, problema=problema, estado=estado, fecha=fecha)
...
def actualizar(id):
    ...
    cursor.execute(sql, datos)
    conexion.commit()
      # Actualizar el campo "detalles" en la tabla de historial
    sql_historial = "UPDATE `historial` SET `detalles` = %s ,`fecha` =%s WHERE `reclamo_id` = %s"
    datos_historial = (_problema,  _fecha,_reclamo_id)
    cursor.execute(sql_historial, datos_historial)
    conexion.commit()
    return redirect('/pendientes')
